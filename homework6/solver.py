...
def timestepPlot(startplot, fileContent, dx_data, dt_data, legend, l):
    fdata = fileContent[startplot].split(", ")
    floatdata = [float(s) for s in fdata]
    plt.plot(dx_data, floatdata)
    index = sequenceGenerator(startplot, len(dt_data)-1)
    legend.append("Timestep %d: %fs" %(index,dt_data[index]))
